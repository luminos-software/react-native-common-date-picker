...
class ListItem extends React.Component {
  ...
  _needSelectedRangeBgColor = (startDate, endDate, currentDate) => {
    ...
    return !!(
      Constants.greaterThan(currentDate, startDate) &&
      Constants.greaterThan(endDate, currentDate)
    );
  };
  ...
  _getMarkTypeStyles = (currentDate, days) => {
    ...
    if (
      listItemStyle.day &&
      typeof listItemStyle.day === "object" &&
      listItemStyle.day.borderRadius
    ) {
    ...
    // Dot
    if (
      selectedDateMarkType === Constants.DEFAULT_DATE_MARK_TYPE.DOT &&
      hasSelected
    ) {
    ...
    if (
      ...
      selectedDateMarkType === Constants.DEFAULT_DATE_MARK_TYPE.SEMIELLIPSE
    ) {
      if (hasSelected) {
        selectedDateStyle.borderRadius = markBorderRadius || 999;
        selectedDateStyle.backgroundColor = selectedDateMarkColor;
        selectedDateRangeStyle.backgroundColor =
          selectedDateMarkType === Constants.DEFAULT_DATE_MARK_TYPE.ELLIPSE
            ? selectedDateMarkRangeColor
            : selectedDateMarkColor;
        if (startDate && endDate) {
          if (startDate === currentDate) {
            selectedDateRangeStyle.borderTopLeftRadius =
              markBorderRadius || 999;
            selectedDateRangeStyle.borderBottomLeftRadius =
              markBorderRadius || 999;
          }
          if (endDate === currentDate) {
            selectedDateRangeStyle.borderTopRightRadius =
              markBorderRadius || 999;
            selectedDateRangeStyle.borderBottomRightRadius =
              markBorderRadius || 999;
          }
        } else {
          selectedDateRangeStyle.borderRadius = markBorderRadius || 999;
  ...
  _renderDays = (day, index, days) => {
    ...
    let disabled = false;
    if (
      Constants.greaterThan(minDate, currentDate) ||
      Constants.greaterThan(currentDate, maxDate)
    ) {
    ...
    return day < 0 ? (
      ...
      >
        <View style={[isDot ? {} : markTypeStyles[0]]}>
          <View
            style={[
              isDot ? { ...CENTER_STYLES, marginTop: 5 } : markTypeStyles[1],
            ]}
          >
            <Text
              style={[
  ...
  render() {
    ...
    const _wks =
      weeksChineseType && weeks === Constants.DEFAULT_WEEK_EN
        ? Constants.DEFAULT_WEEK_ZH
        : weeks;
    const _weeks = Constants.getWeekDays(_wks, firstDayOnWeeks);
    return (
      <View style={[listItemStyles.container, listItemStyle.container || {}]}>
        <ListItemHeader
          item={item}
        ...
        <View
          style={[
            listItemStyles.dayContent,
            { paddingLeft: PADDING / 2 },
            listItemStyle.dayContent || {},
          ]}
        >
          {item.days.map((day, index) =>
            this._renderDays(day, index, item.days)
          )}
        </View>
      </View>
    );
